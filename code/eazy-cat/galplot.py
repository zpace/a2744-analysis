...
def galplot(dir, row, save = False, plot_dir = 'figs'):	
	...
	ax2.text(plotmin + .1*(plotmax-plotmin), 0.55 * np.max(prob), ' $z_{prob}$ = ' + str(np.round(z[np.argmax(prob)], 2)) + '\n $z_{prior}$ = ' + str(np.round(z[np.argmax(prob_prior)], 2)))
	...
	if save == True:
		...
		plt.savefig(plot_dir + '/' + str(row) + '.png')
	else:
		plt.show()
...
def find_highz(dir, zoutname, catname, zllim, zulim):
	...
	f = table.Table.read(zoutname, format = 'ascii')
...
def obj_lookup(dir, obj):
	...
	ds9.set('cmap value 10.090 0.359')
	#ds9.xpa_help('pan')
...
#galplot(1903)
#galplot(66)
#galplot(302) #this is Y1

#obj_lookup('eazy-cat-EA', 1554)
'''
obj_list = filter_sortby_z(dir)
for obj in pbar(obj_list):
	galplot(dir, str(obj), save = True, plot_dir = 'z_5_7_figs')
'''
'''
